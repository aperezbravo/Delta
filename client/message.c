...
int main(void)
{
	...
	int address_translation = inet_aton(SERVER, &server.sin_addr);

	if(address_translation != 0)
	{
		fprintf(stdout, "conversion was successful \n");
	}
	else
	{
		fprintf(stderr, "error in conversion %d \n", address_translation);
		exit(1);
	}
