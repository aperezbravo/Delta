...
int main(void)
{
	...
	//only ourpose is for casting 
	//used by bind()
	
	memset(&hints, 0, sizeof hints);//sets everything to 0's in struct to make sure its empty
	hints.ai_family = AF_INET;//AF_INET refers to IPV4
	hints.ai_socktype = SOCK_DGRAM;//uses the udp connectionless method
	//allows all hosts on the network to connect
	hints.ai_flags = AI_PASSIVE;

	//int socketfd = socket(hints.ai_family, hints.ai_socktype, 0);
